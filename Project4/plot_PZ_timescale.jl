...
P_sample = zeros(length(iterations))
Z_sample = zeros(length(iterations))
...
anim = @animate for (i, iter) in enumerate(iterations)
    ...
    P_sample[i] = file_xz["timeseries/P/$iter"][80, 1, 50];
    Z_sample[i] = file_xz["timeseries/Z/$iter"][80, 1, 50];
...
plot(autocor(P_sample,0:200;))
plot!(autocor(Z_sample,0:200;))